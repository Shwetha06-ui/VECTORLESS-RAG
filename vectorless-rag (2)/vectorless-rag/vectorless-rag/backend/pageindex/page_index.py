...
def extract_matching_page_pairs(toc_page, toc_physical_index, start_page_index):
    pairs = []
    if isinstance(toc_physical_index, dict):
        toc_physical_index = toc_physical_index.get('table_of_contents') or toc_physical_index.get('structure') or (list(toc_physical_index.values())[0] if toc_physical_index else [])
    if isinstance(toc_page, dict):
        toc_page = toc_page.get('table_of_contents') or toc_page.get('structure') or (list(toc_page.values())[0] if toc_page else [])

    if not isinstance(toc_physical_index, list) or not isinstance(toc_page, list):
        return pairs
    ...
    for phy_item in toc_physical_index:
        ...
        for page_item in toc_page:
            if not isinstance(page_item, dict):
                continue
            if phy_item.get('title') and phy_item.get('title') == page_item.get('title'):
...
def add_page_offset_to_toc_json(data, offset):
    ...
    if isinstance(data, list):
        for i in range(len(data)):
            if isinstance(data[i], dict) and data[i].get('page') is not None:
                try:
                    data[i]['physical_index'] = int(data[i]['page']) + offset
                    del data[i]['page']
                except (ValueError, TypeError):
                    pass
    return data
...
def page_list_to_group_text(page_contents, token_lengths, max_tokens=3000, overlap_page=1):    
...
def generate_toc_continue(toc_content, part, model="llama-3.3-70b-versatile"):
...
def process_no_toc(page_list, start_index=1, model=None, logger=None, opt=None):
    ...
    max_tokens = getattr(opt, 'max_token_num_each_node', 3000) if opt else 3000
    ...
    for i, item in enumerate(toc_items):
        ...
        if "physical_index" not in item or item["physical_index"] is None:
            ...
            for j in range(i - 1, -1, -1):
                if isinstance(toc_items[j], dict) and toc_items[j].get('physical_index') is not None:
            ...
            try:
                ...
                if isinstance(first_res, dict) and 'physical_index' in first_res:
                    phy_val = first_res['physical_index']
                    if isinstance(phy_val, str):
                        if phy_val.startswith('<physical_index_'):
                            item['physical_index'] = int(phy_val.split('_')[-1].rstrip('>').strip())
                        elif phy_val.startswith('physical_index_'):
                            item['physical_index'] = int(phy_val.split('_')[-1].strip())
                        elif phy_val.isdigit():
                            item['physical_index'] = int(phy_val)
                    elif isinstance(phy_val, int):
                        item['physical_index'] = phy_val
                    item.pop('page', None)
            except Exception as ex:
                logging.error(f"Error in process_none_page_numbers: {ex}")
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if mode == 'process_toc_with_page_numbers':
        ...
        toc_with_page_number = process_toc_no_page_numbers(toc_content, toc_page_list, page_list, start_index=start_index, model=opt.model, logger=logger, opt=opt)
    else:
    ...
    # Skip verification for speed - trust the TOC extraction
    # For structured documents, this is reliable enough
    if getattr(opt, 'skip_verification', True):
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    # Skip title appearance check for speed - trust the TOC extraction
    if not getattr(opt, 'skip_verification', True):
    ...
    # Skip recursive large node processing for speed
    # This can make many additional LLM calls
    # tasks = [
